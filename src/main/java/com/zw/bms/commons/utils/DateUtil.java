...
public class DateUtil {
    ...
    /**
     * 根据日期字符串获取后一天，返回yyyy-MM-dd格式字符串
     * @param dateStr 格式yyyy-MM-dd
     * @return
     */
    public static String getDayAfter(String dateStr){
    	if(StringUtils.isNotBlank(dateStr)){
    		Date date = DateUtil.getDateFormat(dateStr);
    		if(null != date){
    			return DateUtil.getDateFormat(DateUtil.getDayAfter(date));
    		}
    	}
    	return null;
    }
    ...
    /**
     * 根据日期字符串获取下个月，返回yyyy-MM格式字符串
     * @param dateStr 格式yyyy-MM
     * @return
     */
    public static String getNextMonth2DefaultFormat(String monthStr){
    	if(StringUtils.isNotBlank(monthStr)){
    		Date date = DateUtil.formatDate(monthStr,MONTH_DEFAULT_FORMAT);
    		if(null != date){
    			return DateUtil.getDateFormat(DateUtil.getMonthByRange(date,1), MONTH_DEFAULT_FORMAT);
    		}
    	}
    	return null;
    }
     ...
     * @return Date 
     */
    public static Date getMonthByRange(Date date,int monty) {
    	Calendar c = Calendar.getInstance();
    	c.setTime(date);
    	c.add(Calendar.MONTH, monty);
    	return c.getTime();
     ...
     * 日期格式转换
     * @param date 待转换日期
     * @param sourceFormat 待转换日期格式
     * @param targetFormat 转换后日期格式
     * @return
     */
    public static String converDateFormat(String date,String sourceFormat,String targetFormat){
    	Date sourceDate;
		try {
			sourceDate = new SimpleDateFormat(sourceFormat).parse(date);
